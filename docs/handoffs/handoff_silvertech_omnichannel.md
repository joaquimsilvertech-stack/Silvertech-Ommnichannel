...
>
> **Última atualização:** sessão de 31/07/2026 — Parte 30 concluída em `4dac104`; Parte 31 implementada em `2366720`; **Parte 31A (consolidação da observabilidade dos canais) aplicada localmente e pronta para revisão humana**. Próxima parte do roadmap: **Parte 32** (segurança multi-tenant).
...
**Parte 31 — Observabilidade dos canais WhatsApp.** Implementada no commit `23667206f72494d994ff1d601cf4586d056c6653`: eventos de ciclo de vida e tráfego vinculados a Workspace/canal, integração nos serviços e endpoints de summary/timeseries protegidos para OWNER/ADMIN.

**Parte 31A — Consolidação (aplicada localmente em 31/07/2026).** A fronteira central de gravação agora rejeita canal de outro Workspace; `message_type` usa allowlist e fallback `unknown`; webhook de QR e estado `reconnecting` geram eventos; falhas de restart/reconnect/disconnect/remove geram `CHANNEL_ERROR`; erros de conexão propagam `error_code` sanitizado. O resumo separa inventário atual (`channels_connected`/`channels_disconnected`) de transições históricas (`channel_connected_events`/`channel_disconnected_events`) e inclui `avg_time_to_connection_ms` (CHANNEL_CREATED → primeira CHANNEL_CONNECTED por Workspace/canal; reconexões e pares incompletos são ignorados; a janela considera a primeira conexão). A migration aditiva `0011_aiobservabilityevent_channel_id_snapshot` preserva somente o UUID imutável do canal após hard-delete e faz backfill apenas quando canal e evento pertencem ao mesmo Workspace. Os endpoints da Parte 31 têm parâmetros/respostas tipados no OpenAPI. Testes de contraprova cobrem isolamento, sanitização, idempotência, falhas de gestão, métricas, snapshot, schema e retenção.

**Política de retenção declarada (purge futuro):** eventos de tráfego inbound/outbound de alto volume por **90 dias** (`CHANNEL_OBSERVABILITY_TRAFFIC_RETENTION_DAYS`); ciclo de vida e erros por **365 dias** (`CHANNEL_OBSERVABILITY_LIFECYCLE_RETENTION_DAYS`). Ambos são configuráveis por ambiente. A exclusão automática **não foi implementada** nesta parte e permanece trabalho futuro explícito; até lá, o comportamento de retenção não muda.

> **Ordem apurada do roadmap:** restart/disconnect/remove (P29) → reconnect (P30, concluída) → observabilidade (P31 + P31A, concluídas localmente) → **segurança multi-tenant (P32, próxima)** → Evolution fake/integração (P33).
...
**Baseline da Parte 31:** CI verde informado com **1622 passed** e cobertura de 96,81%. **Validação local final da Parte 31A:** `1638 passed, 1 skipped` no modo padrão do projeto; o único skip é o teste de migration porque `pytest.ini` força `--no-migrations`, e esse teste passou separadamente com `--migrations --create-db` (`1 passed`).
